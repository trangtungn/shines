class CreatorSearch
  ...
  def initialize(search_term)
    ...
    @where_args = {}
    if search_term =~ /@/
      search_by_email_or_name(search_term)
    else
  ...
  def search_by_email_or_name(search_term)
    @where_clause << case_insensitive_search(:first_name)
    ...
    @order = "lower(email) = " + ActiveRecord::Base.connection.quote(search_term) + " desc, first_name, last_name asc"
  end
